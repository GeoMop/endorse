...
def read_konfigurace(file_path):
    ...
    for _, row in konfigurace.iterrows():
        label = row.iloc[col_label]
        orient = row.iloc[col_orientace]
        if str(label).lower() == 'rezerva':
            continue
        labels.append(label)
...
def create_new_sheets_from_jz(data_frames, output_file="piezo4.xlsx", save_to_excel=True):
    if 'JZ' not in data_frames:
        raise ValueError("List 'JZ' nebyl nalezen v datech.")

    df_jz = data_frames['JZ']
...
@measure_time
@save_to_excel_decorator
def log_large_differences(final_data_frames, v_diff, a_diff, rozrazka_file, output_excel_file, output_csv_file, save_to_excel=True):
    columns_to_check = [
                ...
                if j >= 0 and pd.notna(df.loc[j, column]) and pd.notna(df.loc[i + 1, column]):
                    ...
                    if not pd.isna(avg_recent) and not pd.isna(avg_previous):
                        ...
                        if value_diff > v_diff and avg_diff > a_diff:
                            ...
                            elif last_shot_time[column] is not None and 0 < current_time - last_shot_time[column] <= 0.05:
    ...
    if not df_output.empty:
        ...
        print(f"✅ Výsledky uloženy: {output_excel_file}, {output_csv_file}")
    else:
...
labels, orientace = read_konfigurace(script_dir/'measurement_data'/ 'konfigurace_vrtu.xlsx')
# Nacte casy rozrazek, otevře soubor rozrazka
data_s, data_j = read_rozrazka(script_dir/'measurement_data' / 'rozrazka_nova.xlsx')
# Nacte minimální a maximální čas, pro který bude úloha zpracovávána a zda bude zapisovat do excelu
tmin, tmax, zapis_do_souboru, v_diff, a_diff = read_vstupy(script_dir / 'measurement_data' / 'vstup.yaml')


# Hlavni program - úprava dat
# Načtení dat do paměti a zároveň uložení do "piezo3.xlsx"
data_frames = process_piezo_file(
    script_dir / 'measurement_data' / "piezo.xlsx",
...
final_data_frames = log_large_differences(
    final_data_frames,
    v_diff, a_diff,
    rozrazka_file=script_dir / 'measurement_data'/'rozrazka_nova.xlsx',

...
from ..input_data import input_dir, bh_cfg_file
...
def bh_config():
    ...
    """
    return common.config.load_config(bh_cfg_file).boreholes
    #
...
def full_series(zapis_do_souboru):
    ...
    data_frames = add_decimal_time_column(
        data_frames, bh_labels,
        save_to_excel=zapis_do_souboru
    )


    columns, data_sets = read_measurement_data(data_frames, bh_labels)

    # Odstranění sloupců 7 až 19 a uložení výsledku do piezo7.xlsx
    data_frames = remove_columns_from_labels(
        data_frames, bh_labels,
        save_to_excel=zapis_do_souboru
    )
    ...
    data_frames = add_date_columns_to_vystup(
        data_frames,
        save_to_excel=zapis_do_souboru
    )

    # Zapíše tlaky příšlušným čidlům. Prázdné data jsou nahrazeny NaN
    data_frames = merge_columns_to_vystup(
        data_frames, bh_labels, columns,
        save_to_excel=zapis_do_souboru
    )

    # Přejmenuje hlavičky podle názvu vrtu
    data_frames = rename_columns_in_vystup(
        data_frames, bh_labels, columns,
        save_to_excel=zapis_do_souboru
    )

    data_frames = keep_only_vystup(
        data_frames,
        save_to_excel=zapis_do_souboru
    )
...
def process_piezo_file(data_frames):
    ...
    tmin, tmax, v_diff, a_diff = read_vstupy(input_dir / 'vstup.yaml')
...
if __name__ == '__main__':
    zapis_do_souboru = False

    full_dframe = full_series(zapis_do_souboru)
    process_piezo_file(full_dframe)

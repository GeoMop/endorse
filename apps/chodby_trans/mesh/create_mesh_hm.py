...
def make_geometry(factory, cfg:'dotdict', fracture_set):
    ...
    if "drilled_volume" not in cfg_geom.include:
        if "boreholes" in cfg_geom.include:
            ...
            b_storages_fr \
                .set_region(".storages") \
                .mesh_step(cfg.mesh.boreholes_mesh_step)
...
def main(workdir=None):
    ...
    if copy_flag:
        ...
        shutil.copytree(script_path.parent.parent / job.input.dir_path.name, job.input.dir_path, dirs_exist_ok=True)
    ...
    with common.workdir(workdir, clean=False):
        mesh_file = None
        if not Path(cfg.mesh_name + ".msh2").exists():
            mesh_file = make_gmsh(cfg, fracture_set=[], mesh_seed=1)  # use mesh_seed for gmsh randomization
        else:
            mesh_file = File(cfg.mesh_name + ".msh2")
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 2:
        ...
        sys.exit("Provide <workdir>.")

    # common.EndorseCache.instance().expire_all()
    main(workdir=work_dir)

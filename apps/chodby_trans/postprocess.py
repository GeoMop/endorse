...
def select_params(
    ...
    Returns
    -------
    [('S1'|'S2', <index label>, (i_s1,)| (i_s2, j_s2))]
...
def select_sobol(
    param_selection: List[Tuple[str, str, Tuple[int, ...]]],
    ds: xr.Dataset,
) -> xr.Dataset:
    """
    ...
    labels = [label for _, label, _ in param_selection]
    si_rows = [getters[kind](idx) for kind, label, idx in param_selection]

    # --- compute "others" = total - selected ---
    G = ds.dims[group_dim]
    I, J = np.triu_indices(G, k=1)
    sum_S1 = ds["S1"].values.sum(axis=0)               # (*out)
    sum_S2 = ds["S2"].values[I, J, ...].sum(axis=0) if I.size else 0.0
    total_SI = sum_S1 + sum_S2
    if si_rows:
        selected_sum = np.sum(np.stack(si_rows, axis=0), axis=0)
    else:
        selected_sum = 0.0
    others = np.maximum(total_SI - selected_sum, 0.0)   # be resistant to possible loss of precision
    labels.append("others")
    si_rows.append(others)

    # --- assemble SI variable ---
    SI = np.stack(si_rows, axis=0)
    #si_dims = ("group", *out_dims) if out_dims else ("group",)
    ...
    for i, label in enumerate(param_selection):
        if label[0] == "S1":
            ST[i] = ds["ST"].isel({group_dim: label[2][0]}).values
            SI_ci[i] = ds["S1_agg_ci"].isel({group_dim: label[2][0]}).values
            ST_ci[i] = ds["ST_agg_ci"].isel({group_dim: label[2][0]}).values

...
def test_macro_transport():
    # TODO: reduce from 6min down to 30s, reduction of the mesh resolution necessary
   # with common.workdir("sandbox"):
    ...
    seed = 10
    with common.workdir(sandbox_fname(script_dir, f"transport_3d_{seed}"), clean=False):
...
def test_transport_2d():
   # with common.workdir("sandbox"):
    ...
    conf_file = os.path.join(script_dir, "test_data/config.yaml")
    seed = 19
    with common.workdir(sandbox_fname(script_dir, f"transport_2d_{seed}"), clean=False):
        # params for single container source
        cfg = common.load_config(conf_file)
...
def test_fracture_conductivity():
    ...
    fr_family = common.load_config(conf_file)['content']
    ...
    r = 10.
    K = []
    for family in fr_family:
        tra = float(family.tr_a)
        trb = float(family.tr_b)
